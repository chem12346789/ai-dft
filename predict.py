...
for distance in distance_l:
    ...
    rho_grid = np.zeros_like(dm1_r_grid)
    ...
    for iatom, rho_0_grid_atom in enumerate(dm1_r_grid):
        ...
        if atom in args.ai_list:
            ...
            rho_grid[iatom] = result[0, 0, :, :]
            exc_grid[iatom] = result[0, 1, :, :]
            if args.check:
                rho_grid_check = model_dict[f"{atom}-{iatom}-check"][0, :, :]
                mrks_inv.logger.info(f"\npredict vxc {numpy2str(rho_grid)}\n")
                mrks_inv.logger.info(f"\n{numpy2str(rho_grid_check)}\n")
                exc_grid_check = model_dict[f"{atom}-{iatom}-check"][1, :, :]
                mrks_inv.logger.info(f"\npredict exc {numpy2str(exc_grid)}\n")
                mrks_inv.logger.info(f"\n{numpy2str(exc_grid_check)}\n")
        else:
            rho_grid[iatom] = model_dict[f"{atom}-{iatom}"][0, :, :]
            exc_grid[iatom] = model_dict[f"{atom}-{iatom}"][1, :, :]

    rho_grid = rho_grid * dm1_r_grid
    exc_grid = exc_grid * rho_grid
    ...
    if (
        ...
        with open(mrks_inv.path / "energy.json", "r", encoding="utf-8") as f:
            ...
            ccsd_e = data_json["e_cisd"]
            # ccsdt_e = data_json["e_cisdt"]
    else:
        mo = myhf.mo_coeff
    ...
    dm1_r = mrks_inv.grids.matrix_to_vector(rho_grid)
    # dm1_ccsdt_r = mrks_inv.aux_function.oe_rho_r(ccsdt_dm1)

    error_dft = np.sum(abs(dm1_dft_r - dm1_ccsd_r) * mrks_inv.grids.weights)
    error_scf = np.sum(abs(dm1_r - dm1_ccsd_r) * mrks_inv.grids.weights)
