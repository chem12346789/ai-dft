...
def umrks(self, frac_old, load_inv=True):
    """
    Generate 1-RDM.
    """
    ...
    if load_inv and Path(self.data_save_path / "exc_grids.npy").exists():
        ...
        for i_spin in range(2):
            ...
            # ene_vc = np.sum(exc_grids * weights)
            # print(f"Error: {(1e3 * (ene_vc - ene_cc_ele)):.5f} mHa")

            exc_over_rho_grids[i_spin] = exc_grids[i_spin] / (rho_cc[i_spin] + 1e-10)
        ...
        for i in range(2500):
            ...
            for i_spin in range(2):
                for i_batch_grids in range(n_batchs_grids):
                    ...
                    vxc_inv[0, i_slice_grids] -= tmp_grids
                    vxc_inv[1, i_slice_grids] -= tmp_grids
                    del exp_int1e_grids
                    gc.collect()
                    torch.cuda.empty_cache()
            ...
            vj_inv = hybrid(mf.get_jk(self.mol, dm1_inv[0] + dm1_inv[1], 1)[0], vj_inv)
            xc_v = np.array(
                [
    ...
    dft_r = np.array(
        [
            pyscf.dft.numint.eval_rho(self.mol, ao_0, mdft.make_rdm1()[0]) + 1e-14,
            pyscf.dft.numint.eval_rho(self.mol, ao_0, mdft.make_rdm1()[1]) + 1e-14,
        ]
    )
    exc_grids_fake = exc_grids.copy()
    exc_grids_fake1 = exc_grids.copy()
    print("inv_r: ", np.sum(inv_r[0] * weights), np.sum(inv_r[1] * weights))
    print("dft_r: ", np.sum(dft_r[0] * weights), np.sum(dft_r[1] * weights))
