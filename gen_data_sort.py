...
for data_i in args.data:
    ...
    for child in (data_path).glob("*"):
        ...
        if (
            np.abs(int(float(child.parts[-1]) * 10) * 0.1 - float(child.parts[-1]))
            > 1e-3
        ):
            continue
        ...
        if (
            ...
            masks_v_sort = np.zeros_like(masks_v, dtype=float)
            for i in range(data_weit.shape[0]):
                for j in range(data_weit.shape[1]):
                    ...
                    masks_v_sort[i, j, :] = masks_v[i, j, data_sort_index[i, j, :]]
            ...
            if args.energy:
                ...
                masks_e_sort = np.zeros_like(masks_e, dtype=float)
                for i in range(data_weit.shape[0]):
                    for j in range(data_weit.shape[1]):
                        masks_e_sort[i, j, :] = masks_e[i, j, data_sort_index[i, j, :]]
            ...
            for i in range(data.shape[0]):
                ...
                masks_ve[0, :, :] = masks_v_sort[i, :, :]

                if args.energy:
                    if args.nclass == 1:
                        if len(masks_e_file) == 1:
                            masks_ve[0, :, :] = masks_e_sort[i, :, :]
                    elif args.nclass == 2:
                        masks_ve = np.zeros((2, data.shape[1], data.shape[2]))
                        if len(masks_e_file) == 1:
                            masks_ve[0, :, :] = masks_v_sort[i, :, :]
                            masks_ve[1, :, :] = masks_e_sort[i, :, :]

                np.save(imgs_path / data_name, data_sort[i, :, :])
                np.save(mask_path / data_name, masks_ve)
                np.save(weit_path / data_name, weight[i, :, :])
